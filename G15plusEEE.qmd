...
g <- ggplot(x, aes(year, n / ntot, succ = n, fail = ntot - n)) +
  ...
  geom_smooth(method = "gam", colour = "forestgreen", formula = cbind(succ, fail) ~ s(x, k = 3, m = 2), method.args = list(family = "binomial"), linewidth = 3) +
  labs(x = "Années", y = "Nb. d'obs. / Nb. total d'obs.", title = "Évolution du taux d'observations de EEE") +
  ylim(c(0, 0.05)) +
  theme_light()
g

ggsave(file = "G15plus_EEE_tauxobservations.svg", plot = g, width = 5, height = 4)
