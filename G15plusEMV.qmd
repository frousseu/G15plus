...
tsrank2 <- function(x, group){
  ...
  lines(as.Date(rd), scales::rescale(cu, to = c(-3, -2)), lwd = 2, col = "forestgreen")
  ####
...
#split(rp, rp$SNAME)
names(rp)[which(names(rp) == "Ancienne valeur")] <- "SRANK"
names(rp)[which(names(rp) == "Date ancienne valeur")] <- "ancienne_valeur"
names(rp)[which(names(rp) == "Date changement")] <- "date"
rp[ , bd := "passé"]

keepp <- c("SNAME", "SCOMNAME", "GRANDGROUPE", "SRANK", "date", "ancienne_valeur", "bd")
keepa <- keepp[-(length(keepp)-1)]

r <- rbind(ra[, ..keepa], rp[, ..keepp], fill = TRUE)
r <- r[ ,.(SNAME, GRANDGROUPE, bd, SRANK, date, ancienne_valeur)]
r[ , date := as.Date(date)]
r[ , ancienne_valeur := as.Date(ancienne_valeur)]
r <- r[order(SNAME, -bd, date), ]
tab <- rev(sort(table(r$SRANK)))#; tab
l <- split(r[, -2], r$SNAME)
tab
...
Le rang actuel est plus précaire que le dernier rang passé.

```{r, echo = FALSE}
l[["Sternotherus odoratus"]]
```

##### 

Le rang actuel est moins précaire que le dernier rang passé.
...
Le dernier rang passé et le rang actuel ne correspondent pas du tout.
...
Le rang actuel ne correspond pas à un rang passé à la même date (2008-01-21).

```{r, echo = FALSE}
l[["Erigeron hyssopifolius"]]
```

##### 

On ne sait pas quel rang attribuer pour la période allant de 2004-11-19 à 2019-08-06.
...
Le dernier rang passé et le rang actuel ne correspondent pas et il y a une incertitude entre 2009-08-24 et 2015-03-30.

```{r, echo = FALSE}
l[["Vermivora cyanoptera"]]
```
