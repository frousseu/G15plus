...
g <- ggplot(x, aes(date, diff)) +
  ...
  geom_smooth(colour = "forestgreen", linewidth = 3) +
  facet_grid(cols = vars(group), rows = vars(rank), scales = "fixed") +
  ylim(c(-0.5, 0.5)) +
  labs(x = "Années", y = "Différence par rapport au rang initialement attribué", title = "Évolution des rangs de précarité") +
  theme_light()
...
ggsave(file = "G15plus_EMV_précarité.png", plot = g, width = 10, height = 6)
